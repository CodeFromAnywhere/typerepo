...
export var SwipeHomepage = function (props) {
    ...
    var buttonClass = "bg-gray-200/60 rounded-full hover:bg-white cursor-pointer text-black p-2";
    return (React.createElement(Div, null,
        React.createElement(Div, { className: "h-screen overflow-auto snap-mandatory snap-y" }, items.map(function (item, index) {
            ...
            var nameSizeClass = item.title && item.title.length > 10 ? "text-2xl" : "text-6xl";
            return (React.createElement(Div, { key: "item".concat(index), style: {
                ...
                item.title ? (React.createElement(ALink, { href: item.href || "#", className: "absolute top-20 left-20 font-bold drop-shadow bg-green-500/20 rounded-md px-4 py-2 backdrop-blur-sm ".concat(nameSizeClass, " ").concat(item.href ? "cursor-pointer" : "cursor-default") }, item.title)) : null,
        ...
        React.createElement(Div, { className: "z-index-1 absolute bg-gradient-to-b via-blue-300/50 from-white/50 to-yellow-300 rounded-full w-12 h-12 top-4 left-4" },
